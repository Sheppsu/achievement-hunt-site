# taken from https://github.com/osuAkatsuki/bancho.py
...
if TYPE_CHECKING:
    from .models import Match, PlaytestAccount
    ...
    def __next__(self) -> BasePacket:
        ...
        while self.body_view:  # len(self.view) < 7?
            ...
            if p_type not in self.packet_map:
                print("Packet type not handled:", p_type.name)
...
@cache
def bot_stats(player: PlaytestAccount) -> bytes:
...
def user_stats(player: PlaytestAccount) -> bytes:
...
@cache
def bot_presence(player: PlaytestAccount) -> bytes:
    return write(
        ServerPackets.USER_PRESENCE,
        (player.user.id, osuTypes.i32),
        (player.user.username, osuTypes.string),
        (-5 + 24, osuTypes.u8),
...
def user_presence(player: PlaytestAccount) -> bytes:
    return write(
        ServerPackets.USER_PRESENCE,
        (player.user.id, osuTypes.i32),
        (player.user.username, osuTypes.string),
...
# packet id: 88
def match_invite(player: PlaytestAccount, target_name: str) -> bytes:
    assert player.match is not None
    msg = f"Come join my game: {player.match.embed}."
    return write(
        ServerPackets.MATCH_INVITE,
        ((player.user.username, msg, target_name, player.id), osuTypes.message),
    )
